...
class Generator(nn.Module):
    def __init__(self, input_channel:int) -> None:
        ...
        for i in range(3):
            if i == 0:
                layers[f'down_{i}'] = self._make_conv_block(input_channel, channel, 7, 1, 3, 'd')
            else:
    ...
    def _make_conv_block(self, in_channel:int, out_channel:int,
        ...
        layer = []
        if mode.lower() == 'd':
            layer.append(nn.Conv2d(in_channel, out_channel, kernel, stride, padding, bias=False))
        if mode.lower() == 'u':
            layer.append(nn.ConvTranspose2d(in_channel, out_channel, kernel, stride, padding, bias=False))
...
class Discriminator(nn.Module):
    def __init__(self, input_channel:int, height:int, width:int, n_domain:int) -> None:
        ...
        channel = 64
        layers['input'] = self._make_conv_block(input_channel, channel, 4, 2, 1)
        ...
        self.src = nn.Conv2d(channel, 1, 3, 1, 1, bias=False)
        self.cls = nn.Conv2d(channel, n_domain, (self.height, self.width), 1, 0, bias=False)
